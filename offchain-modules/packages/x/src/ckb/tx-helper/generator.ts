import { Cell, Script, Indexer, WitnessArgs, utils, blockchain } from '@ckb-lumos/base';
  ...
  async mint(records: MintAssetRecord[], indexer: Indexer): Promise<TransactionSkeletonType> {
    for (;;) {
      try {
        ...
        const mintWitnessArgs = blockchain.WitnessArgs.pack({
        ...
        txSkeleton = txSkeleton.update('witnesses', (witnesses) => {
          ...
          const witnessArgs = blockchain.WitnessArgs.unpack(witnesses.get(0) as string);
          ...
          return witnesses.set(0, bytes.hexify(blockchain.WitnessArgs.pack(newWitnessArgs)));
        });
    ...
    if (
      ...
      const messageToSign = (() => {
        ...
        const rawTxHash = utils.ckbHash(
          bytes.hexify(blockchain.RawTransaction.pack(createTransactionFromSkeleton(txSkeleton))),
        );
